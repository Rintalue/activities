  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
      <.input field={@form[:room_id]} type="select"  label="Choose Room" options={@rooms} prompt="choose room" />
        <.input
          field={@form[:type]}
          type="select"
          label="Type of Activity"
          options={["Operations","Cleaning","Non-Serialized Cleaning", "Maintenance"]}
          prompt="choose activity"
        />
        <.input field={@form[:product_id]} type="text" label="  Product ID" id="start-time" />
        <.input field={@form[:batch_number]} type="text" label="Batch Number" id="start-time" />
        <.input field={@form[:start_time]} type="datetime-local" label="Start time" id="start-time" />


        <.input field={@form[:stop_time]} type="datetime-local" label="Stop time" id="stop-time" />
        <.input field={@form[:duration]} type="text" label="Duration in Minutes" id="start-time" />
